...
module.exports = {
  ...
  author: 'Dale Mekumi', 
  usage: 'video songtitle',
  async execute(senderId, args, pageAccessToken, sendMessage) {

    const prompt = args.join(' ');
    if (!prompt) return sendMessage(senderId, { text: "𝑼𝒔𝒂𝒈𝒆: video 𝒕𝒊𝒕𝒍𝒆" }, pageAccessToken);
    ...
    const response = await axios.get(`https://search.iyot.plus/ytsearch?title=${encodeURIComponent(prompt)}`);
      ...
      const responseTime = new Date().toLocaleString('en-US', { timeZone: 'Indian/Antananarivo', hour12: true });
      
      
      sendMessage(senderId, { 
        text: `𝗬𝗼𝘂𝘁𝘂𝗯𝗲 𝗗𝗼𝘄𝗻𝗹𝗼𝗮𝗱𝗲𝗿\n\n🤖𝑻𝒊𝒕𝒍𝒆: ${title}\n\n𝑼𝒓𝒍: ${url}\n\n⏰ Indian/Antananarivo: ${responseTime}\n\n𝑫𝒐𝒘𝒏𝒍𝒐𝒂𝒅𝒊𝒏𝒈 𝑺𝒐𝒏𝒈 𝑷𝒍𝒆𝒂𝒔𝒆 𝑾𝒂𝒊𝒕...` 
      }, pageAccessToken);
